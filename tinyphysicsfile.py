...
def run_rollout_with_pid(data_path, model_path, p, i, d, debug=False):
    # Initialize the simulation model
    tinyphysicsmodel = TinyPhysicsModel(model_path, debug=debug)
    
    # Create a PID controller with specified parameters
    controller = PIDController(p=p, i=i, d=d)
    
    # Initialize simulator with custom controller
    sim = TinyPhysicsSimulator(tinyphysicsmodel, str(data_path), controller=controller, debug=debug)
    
    # Run the rollout and return the results
    return sim.rollout()  # returns cost dictionary, etc.
